<template>
  <div class="abs-center">
    <form @submit.prevent="handleLogin">
      ...
      <input type="text" name="user" id="user" class="form-control" v-model="userName" placeholder="Usuario" required />
      <label for="password" style="font-weight: bold">Contraseña:</label>
      <input type="password" name="password" id="password" class="form-control" v-model="password"
        placeholder="Password" required />

      <button type="submit"
        class="w-full my-4 bg-transparent hover:bg-blue-500 text-blue-700 font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent rounded">Login</button>
      <p>
        ¿No tienes cuenta?
